package fxgeditor.parser.utils 
{
	...
	public class GeomUtil
	{
		...
		public static function getBezierPointsOnRect( rect:Rectangle , topLeftRadius:Point,
													  topRightRadius:Point, bottomLeftRadius:Point, 
													  bottomRightRadius:Point ):Vector.<Point>
		{
			
			var b:Point = new Point();
			
			var tl:Point = new Point( rect.x  , rect.y + topLeftRadius.x );
			var tlc:Point = new Point( rect.x  , rect.y );
			var tl1:Point = new Point( rect.x + topLeftRadius.x , rect.y );
			
			var tr:Point = new Point( rect.x + rect.width - topRightRadius.x , rect.y );
			var trc:Point = new Point( rect.x + rect.width , rect.y );
			var tr1:Point = new Point( rect.x + rect.width , rect.y + topRightRadius.x );

			var br:Point = new Point( rect.x + rect.width  , rect.y + rect.height - bottomRightRadius.x );
			var brc:Point = new Point( rect.x + rect.width  , rect.y + rect.height );
			var br1:Point = new Point( rect.x + rect.width - bottomRightRadius.x   , rect.y + rect.height );
			
			var bl:Point = new Point( rect.x + bottomLeftRadius.x , rect.y + rect.height);
			var blc:Point = new Point( rect.x , rect.y + rect.height);
			var bl1:Point = new Point( rect.x  , rect.y + rect.height - bottomLeftRadius.x );
			
			return Vector.<Point>([ tl1, b , tr ,tr, trc, tr1, 
									tr1, b, br,br, brc, br1, 
									br1, b, bl, bl , blc , bl1, 
									bl1, b, tl ,tl, tlc, tl1, tl1 ]);
		}
