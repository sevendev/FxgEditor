package fxgeditor.parser.path 
{
	...
	public class BezierCurve implements ICurve
	{
		...
		public static const MAX_STANDARD_DETAIL_LENGTH:Number = 300;
		...
		{
			var cx1:Point = new Point( bp1.cx1, bp1.cy1 );
			var cx:Point = new Point( bp2.cx , bp2.cy  ) ;
			return  GeomUtil.getDistance( bp1.asPoint , cx1 ) +
					GeomUtil.getDistance( cx1 , cx ) +
					GeomUtil.getDistance( bp2.asPoint , cx );
		}
		...
		private function _bezierCurve( x0:Number, y0:Number, cx0:Number, cy0:Number, cx1:Number, cy1:Number, x:Number, y:Number ):GraphicsPath
		{
			...
			var detail:int = Math.max( getLength() / MAX_STANDARD_DETAIL_LENGTH * Constants.BEZIER_DETAIL , Constants.BEZIER_DETAIL );
			
			var k:Number = 1.0/ detail;
			var t:Number = 0;
			var tp:Number;
