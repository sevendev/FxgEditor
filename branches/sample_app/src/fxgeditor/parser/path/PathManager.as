package fxgeditor.parser.path 
{
	...
	public class PathManager extends EventDispatcher
	{
		...
		private var _lastStartPoint:IEditPoint;
		...
		public function closePath():void 
		{
			...
			var first:IEditPoint = _lastStartPoint;
			var last:IEditPoint = getLastPoint();
		...
		{
			if ( !_editPoints.length ) return null;
		...
		public function getCommands():GraphicsPath 
		{
			...
			for each ( var p:IEditPoint in _editPoints ) 
			{
				if ( p.isMoveTo() ) 
				{
					commands = commands.concat( Vector.<int>( [ GraphicsPathCommand.MOVE_TO ] ) );
					vertices = vertices.concat( Vector.<Number>( [ p.ax, p.ay ]) );
				}
				else if ( last ) 
				{
		...
		public function getXMLAttr():String 
		{
			if ( !getFirstPoint() ) return "";
			var str:String = "";
			var last:IEditPoint;
			for each ( var p:IEditPoint in _editPoints ) 
			{
				if ( p.isMoveTo() ) 
					...
					str += new CurveFactory( last , p ).getSvgAttr();
