package fxgeditor.ui 
{
	...
	public class GradientBox extends Sprite
	{
		...
		{ 
			...
			grad = editStroke ? this.editItem.style.stroke as IGradient : this.editItem.style.fill as IGradient;
			if ( !grad ) return;
		...
		private function createLinearBox():void 
		{
			...
			if ( !lg.p1 || !lg.p2  ) 
			{
				pt1 = editItem.style.bounds.topLeft;
				pt2 = pt1.clone();
				pt2.offset( editItem.style.bounds.width, editItem.style.bounds.height );
			}else 
			{
				pt1 = GeomUtil.getVirtualPosition( this , editItem.asContainer , lg.p1.x, lg.p1.y );
				pt2 = GeomUtil.getVirtualPosition( this , editItem.asContainer , lg.p2.x, lg.p2.y );
			}
		...
		private function createRadialBox():void 
		{
			...
			if ( !rg.p1 || !rg.p2 || !rg.p3  ) 
			{
				pt1 = editItem.style.bounds.topLeft;
				pt1.offset( editItem.style.bounds.width / 2 , editItem.style.bounds.height / 2 );
				pt2 = pt1.clone();
				pt2.offset( 0, editItem.style.bounds.height / 2 );
				pt3 = pt1.clone();
				pt3.offset( editItem.style.bounds.width / 2, 0 );
			}else 
			{
				pt1 = GeomUtil.getVirtualPosition( this , editItem.asContainer , rg.p1.x, rg.p1.y );
				pt2 = GeomUtil.getVirtualPosition( this , editItem.asContainer , rg.p2.x, rg.p2.y );
				pt3 = GeomUtil.getVirtualPosition( this , editItem.asContainer , rg.p3.x, rg.p3.y );
			}
			
			createIcon( pt1.x, pt1.y , true  );
			createIcon( pt2.x, pt2.y );
			createIcon( pt3.x, pt3.y );
			draw();
		}
		...
		private function drawRadial():void 
		{
			...
			this.graphics.lineTo( icons[1].x , icons[1].y );
			this.graphics.moveTo( icons[0].x , icons[0].y );
			this.graphics.lineTo( icons[2].x , icons[2].y );
			
			var pt1:Point = GeomUtil.getActualPosition( this , editItem.asContainer , icons[0].x, icons[0].y );
			var pt2:Point = GeomUtil.getActualPosition( this , editItem.asContainer , icons[1].x, icons[1].y );
			var pt3:Point = GeomUtil.getActualPosition( this , editItem.asContainer , icons[2].x, icons[2].y );
			var rg:RadialGradient = grad as RadialGradient;
		...
		private function createIcon( x:Number , y:Number , main:Boolean = false ):void 
		{
			...
			icons.push( icon );
			if ( main )
				icon.addEventListener( MouseEvent.MOUSE_DOWN , dragAll );
			else
		...
		private function dragAll( e:MouseEvent ):void 
		{
